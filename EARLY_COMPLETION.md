...
Early completion = transition a flow run to `completed` with output supplied by an explicit early-completion signal, then cancel remaining pending/queued/running work.

## Why separate from cancellation

- Cancellation ends as `canceled` and produces no normal business output.
- Early completion ends as `completed` and **does** produce business output.

## Scope (MVP)

### In scope
1. Flow-only early completion (tasks out of scope).
2. Explicit output payload for completion.
3. Stop remaining step/map execution after early completion decision.
4. Idempotent conflict handling between early-completion and normal completion/failure.

### Out of scope (MVP)
- Automatic early-completion condition DSL in SQL.
- Multiple competing early-completion values merge semantics.
- Task-level early-completion API.
...
Step/map tables add `canceled` status support (shared with cancellation MVP), because leftover work is canceled after early completion.
...
func CompleteFlowEarly(output any, reason string) error
```

When step handler returns `CompleteFlowEarly(...)`:
...
- If another worker already completed/failed/early-completed flow, `cb_early_exit_flow` returns `changed=false`.
- Worker treats this as benign race outcome and exits normally.

## Guarding normal step completion

`cb_complete_step` and `cb_fail_step` already gate on parent status; ensure they only mutate parent flow when parent still `started`. This prevents post-exit writes from overriding early-completion.

## Go API

## Optional explicit client API
```go
func (c *Client) EarlyCompleteFlow(
    ctx context.Context,
    flowName string,
    runID int64,
    stepName string,
    output any,
    opts ...EarlyCompletionOpts,
) error
```

```go
type EarlyCompletionOpts struct {
    Reason string
}
```

MVP can ship only handler-driven early completion first; client API may come later.

## Type/contract semantics

- Early-completion output must be compatible with flow `WaitForOutput` decode target.
- Conceptually, early-completion output stands in for final-step output.
- Document that when `early_exited=true`, final-step handler may not have run.

## Interaction with cancellation

- If flow already `canceling/canceled`, early completion must not apply (`changed=false`).
- If early completion wins first, subsequent cancel is no-op on terminal completed run.

## Idempotency and race rules

1. Early completion is idempotent for a run: first winner decides output.
...
Dashboard hints:
- badge: “Completed (early completion)”
- show deciding step and reason

## Test matrix

1. Step triggers early completion -> flow completed with expected output.
2. Parallel branch still running when early completion occurs -> branch stops/cancels, no output override.
3. Race: normal final completion vs early completion -> exactly one winner, stable output.
4. Race: fail vs early completion -> exactly one terminal state.
5. WaitForOutput decodes early-completion output correctly.
6. Duplicate early-completion attempts are no-op.
...
4. Add API exposure for early-completion metadata.
5. Add integration race tests.

## Relation to event triggering

- Event triggering decides when flow runs begin.
- Early completion decides that a triggered run can complete sooner with a valid final output.
