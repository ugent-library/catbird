...
func TestBindExactTopic(t *testing.T) {
	...
	if err := client.Dispatch(ctx, "events.user.created", map[string]string{"id": "123"}); err != nil {
		t.Fatal(err)
	}
...
func TestBindSingleTokenWildcard(t *testing.T) {
	...
	for _, tc := range testCases {
		if err := client.Dispatch(ctx, tc.topic, map[string]string{"test": "data"}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestBindFanout(t *testing.T) {
	...
	if err := client.Dispatch(ctx, "broadcast.message", payload); err != nil {
		t.Fatal(err)
	}
...
func TestUnbind(t *testing.T) {
	...
	if err := client.Dispatch(ctx, "test.topic", "before unbind"); err != nil {
		t.Fatal(err)
	}
	...
	if err := client.Dispatch(ctx, "test.topic", "after unbind"); err != nil {
		t.Fatal(err)
	}
...
func TestBindPrefixOptimization(t *testing.T) {
	...
	// Should match
	if err := client.Dispatch(ctx, "very.long.prefix.path.to.resource.created", "match"); err != nil {
		t.Fatal(err)
	}

	// Should not match (different prefix)
	if err := client.Dispatch(ctx, "very.long.different.path.to.resource.created", "nomatch"); err != nil {
		t.Fatal(err)
	}
...
func TestBindOrderIndependence(t *testing.T) {
	...
	for _, topic := range topics {
		if err := client.Dispatch(ctx, topic, "test"); err != nil {
			t.Fatal(err)
		}
	}
...
func TestBindRebind(t *testing.T) {
	...
	// Dispatch once
	if err := client.Dispatch(ctx, "test.topic", "data"); err != nil {
		t.Fatal(err)
	}
...
func TestBindEmptyPrefix(t *testing.T) {
	...
	if err := client.Dispatch(ctx, "foo.bar", "test"); err != nil {
		t.Fatal(err)
	}
