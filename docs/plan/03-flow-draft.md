...
> Rewritten 2026-07-14 (branch experiment), replacing both the 2026-07-13 draft
> and the earlier revision of this draft. Two moves since then, decided in
> review (decision log D34, D38): the engine no longer rides the stream layer
...
- `cb_flow_defs (name PK, first_step, steps jsonb, on_fail, retention NOT
  ...
  `steps`. Not `cb_flows`: the old schema's static registry has that exact
  name in every old-catbird database, and old and new must coexist.
...
- `cb_flow_schedules (PK (flow, name), every, catch_up, input, next_at)` —
  cron templates for scheduled runs, same shape and define semantics as the
  stream layer's schedule table, delivered by the flow tick job: a due row
  calls `cb_flow_run(flow, input)` and re-arms `next_at` in one transaction —
  exactly-once per slot by construction, no message, no translation.
...
- **`cb_flow_define(flow, first_step, steps, on_fail, retention)`** plus the
  ...
  (D26), guarded so unchanged declarations write nothing. The Go
  `flow.Define` performs them in one advisory-locked transaction. Deploy-time
  setup, not part of the worker contract.
...
- **`cb_flow_complete(run_id, step_id, attempt, output, spawns, run_output)
  ...
  validate and insert the spawned steps (names against `cb_flow_defs.steps`,
...
recorded in `cb_flow_defs.first_step`; so is the failure step — `OnFail`
names one recorded in `cb_flow_defs.on_fail`.
...
**The worker** is flow's own small loop sharing the stream loop's D27
skeleton (one select owns handler-done, the extend tick and cancellation):
claim a batch → per step: start → handler → complete/fail — extending on the
cadence, releasing unstarted leases on shutdown. Handlers are registered per
...
Terminal runs are pruned past `cb_flow_defs.retention` (NOT NULL, default 30
...
6. Tests, the semantic core:
   ...
   - define convergence: changed terms, steps map and schedules apply on
     redeploy; unchanged define writes nothing.
