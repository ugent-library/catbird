...
10. **Workflow progress and results.** A caller holding only the id `Enqueue` returned can ask nothing of the whole workflow: is anything still going to run, did anything die, what results are there. The reads are settled and cheap; the API shape is not, and deciding it is part of the item. See "Workflow progress and results" below.
...
**Workflow progress and results.** A caller that starts a workflow holds one id — the one `Enqueue` returned — and cannot predict the workflow's shape, because handlers decide the fan-out as they run. Every read today wants a coordinate that caller does not have: `Status` takes one job's id, `Output` and `Outputs` take a job type. There is no way to ask of the whole workflow: is anything still going to run, did anything die, and what results are there.

Two facts already in place make the answer cheap. The workflow is flat: the completion stamps every new claim with `coalesce(group_id, message_id)` of the completing job, so a grandchild carries the root id and one id addresses the whole tree, however deep it grew. And children are created in the statement that deletes the parent's claim, so a workflow never has zero live claims with more work coming — "no claims left" is a sound completion test, where a system whose handlers complete first and insert children afterwards would read done mid-flight.

So it is two reads and no new column. A progress read counts the workflow's claims — still to run, of those how many are parked on a signal, dead, plus the first dead job's id and error so `Status` on that id has the detail — as a `UNION ALL` of the group-index probe and the primary-key probe for the root, the two-probe shape `Status` already has; the root row in `cb_messages` separates completed from never-existed, as in `Status`. A results read returns every row of `cb_outputs` under the workflow id — job id, job type, payload — one probe on `cb_outputs_group_idx`, which already leads with `group_id`. One index changes: `cb_claims_group_idx` drops `died_at IS NULL` from its predicate so dead jobs are countable without a scan. `group_id IS NOT NULL` stays, and it is the whole size argument — single-shot jobs stay out, and dead rows are few and leave with `GC`.

Two limits to state rather than fix. Progress is a live view, not a record: `GC` removes dead claims and then messages, so a workflow that failed eventually reads as completed and later as not found — the lifetime `StatusCompleted` already has. And a job that completed without calling `SetOutput` leaves no trace it was ever in the workflow, so "which steps ran" stays unanswerable; the fix would be `group_id` on `cb_messages`, eight bytes on every message row — published messages and single-shot jobs included — to serve a read nothing makes yet, so it waits for a caller.

The open part is the API, and deciding it is most of the item. The new reads would inherit two frictions the surface already has. A bare `int64` is a job id in `Status` and a workflow id in `Signal`, `Cancel`, `Output` and `Outputs`, and nothing in the types says which; for a single-shot job the two are equal, so the mistake stays quiet until the first workflow, where `Cancel` on a child's id cancels one job and `Outputs` on it returns nothing. And `Output` unmarshals into `dest` while `Outputs` and `DependencyOutputs` return raw JSON — two conventions for one value. A handle type carrying the workflow id, on the `Cursor` precedent, was sketched and did not convince; whether the answer is a handle, more package functions, or renaming what exists is undecided, and the vocabulary split — `group_id` in the schema, workflow in this document, `groupID` in the parameters — gets settled with it.
