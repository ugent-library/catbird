...
12. **Metrics and telemetry.** What the queues are doing, readable without writing SQL: depth and age of the oldest ready job per queue, throughput, failures, dead jobs. The shape is undecided — counters the process exports, a read like `Status` for a whole queue, or a documented set of queries — and deciding that is part of the item. It comes last so it reads a finished surface: the item above may move statements into the database, and workflow progress adds reads that metrics would count.
