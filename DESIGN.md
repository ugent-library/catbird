...
**A deduplication key lives as long as its message, and so does a result.** `GC` deletes messages by age and frees their keys with them, so a key stops deduplicating once its message ages out — fine for `cron:<name>:<minute>`, but retention has to exceed the window for any key used for idempotency. `cb_outputs` cascades from `cb_messages` the same way, so retention also has to exceed the longest workflow or a late job cannot read an early one's result.
...
5. **The cron helper**, on `DeduplicationKey` alone.
...
**Cron helper.** `RunCron(ctx, pool, name, every, jobType)` enqueues on the interval and once at start, so applications do not each rebuild the key format, which every client must produce identically. It sets `DeduplicationKey` to `cron:<name>:<minute>` and puts the minute in the payload. The key deduplicates the tick, not the run: several processes ticking in the same minute produce one job, and a run that outlives its interval does not stop the next minute's job. A handler that must not run beside itself opens with `pg_try_advisory_lock` on its own name and returns when it does not get the lock. The lock skips overlap and nothing more — the ticks that queued up behind a long run still run one at a time afterwards — so a handler whose late runs are waste also compares the payload's minute with the clock and returns on a stale one, the same test a process that wakes late applies before enqueueing.
